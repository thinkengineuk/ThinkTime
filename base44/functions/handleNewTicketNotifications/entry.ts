...
Deno.serve(async (req) => {
  try {
    ...
    // Build unique recipient list (admins + assigned agent)
    const recipientSet = new Set(adminAgentEmails);
    if (assigned_agent_email) recipientSet.add(assigned_agent_email);
    const allRecipients = Array.from(recipientSet);

    if (allRecipients.length === 0) {
      console.log('No recipients found for new ticket notification');
    ...
    const metaTable = `
      <div style="background:#f8fafc;border-radius:8px;border:1px solid #e2e8f0;padding:16px 20px;margin-bottom:20px;">
        <table width="100%" cellpadding="0" cellspacing="0">
          ...
          ${ticketMetaRow('Priority', `${priorityDot}<span style="color:${priorityColor(priority)};font-weight:600;">${priority || 'medium'}</span>`)}
          ${ticketMetaRow('Category', category || 'general')}
          ${assigned_agent_name ? ticketMetaRow('Assigned To', assigned_agent_name) : ''}
        </table>
      </div>
      ${description ? `
      <div style="margin-bottom:20px;">
        ...
        <div style="background:#f8fafc;border-left:3px solid #0ea5e9;border-radius:0 6px 6px 0;padding:12px 16px;font-size:14px;color:#334155;line-height:1.7;">${description.replace(/\n/g, '<br/>')}</div>
      </div>
      ` : ''}
    ...
    console.log(`New ticket notifications sent to: ${allRecipients.join(', ')}`);
    return Response.json({ success: true, sentTo: allRecipients });
  } catch (error) {
    console.error('Error in handleNewTicketNotifications:', error);
