...
Deno.serve(async (req) => {
  try {
    ...
    if (!isAgent) return Response.json({ error: 'Forbidden' }, { status: 403 });

    const firstName = agent_name ? agent_name.split(' ')[0] : 'there';
    ...
    const bodyHtml = `
      <p style="margin:0 0 20px 0;font-size:14px;color:#475569;">Hi ${firstName}, a new ticket has been assigned to you.</p>
    ...
    await base44.asServiceRole.integrations.Core.SendEmail({
      to: agent_email,
      subject: `[${displayId}] Assigned: ${subject}`,
      body: html
    });
